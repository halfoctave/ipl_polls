...
def combine_csv_files(input_dir):
    """
    Combine all margin poll CSV files for a given week into a single leaderboard CSV, sorted by Total_Points.
    Adds Dense Rank and Standard Rank columns.
    
    Args:
        input_dir (str): Path to the directory containing weekly margin poll CSV files
    Returns:
        str: Path to the combined output CSV file
    """
    base_dir = os.path.dirname(__file__)
    result_dir = os.path.abspath(os.path.join(base_dir, "../results/weekly/poll_margin"))
    ...
    match_number = 1
    all_usernames = set()
    required_headers = {'Username', 'Display Name', 'Margin Voted Short', 'Points'}
        ...
        try:
            with open(file_path, newline='', encoding='utf-8-sig') as csvfile:
                ...
                for row in reader:
                    ...
                    margin_short = row.get('Margin Voted Short', '---')
                    try:
                    ...
                    user_data[username][f'Match_{match_number}_Margin_Short'] = margin_short
                ...
                for username in all_usernames:
                    if f'Match_{match_number}_Margin_Short' not in user_data[username]:
                        user_data[username][f'Match_{match_number}_Margin_Short'] = '---'
    ...
    fieldnames = ['Dense Rank', 'Standard Rank', 'Username', 'Display Name'] + \
                 sum([[f'Match_{i}_Margin_Short', f'Match_{i}_Points'] for i in range(1, match_number)], []) + \
                 ['Total_Points']
    ...
    print(f"Weekly margin leaderboard created: {output_file}")
    print(f"Processed {match_number-1} matches, {len(leaderboard_list)} users")
    return output_file

if __name__ == "__main__":
    base_dir = os.path.dirname(__file__)
    input_dir = os.path.abspath(os.path.join(base_dir, "../data/processed", WEEK, "poll_margin"))
    output_file = combine_csv_files(input_dir)
    if output_file:
        print(f"Weekly margin leaderboard generation completed for {WEEK}")