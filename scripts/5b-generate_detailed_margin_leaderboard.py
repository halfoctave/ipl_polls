...
def generate_detailed_margin_leaderboard():
    """
    Generate a detailed leaderboard CSV combining margin poll matches up to the specified week.
    Includes per-match margin selections and points.
    ...
    input_dir = os.path.abspath(os.path.join(base_dir, '../results/weekly/poll_margin'))
    output_dir = os.path.abspath(os.path.join(base_dir, '../results/detailed/poll_margin'))
        ...
        try:
            with open(file, 'r', encoding='utf-8-sig') as f:
                ...
                if not sample_row or 'Total_Points' not in sample_row:
                    ...
                    continue
                match_count = sum(1 for key in sample_row if key.startswith('Match_') and 'Margin_Short' in key)
                f.seek(0)
                reader = csv.DictReader(f)
                ...
                for row in reader:
                    ...
                    for match_num in range(1, match_count + 1):
                        ...
                        short_key = f'Match_{match_num}_Margin_Short'
    ...
    for i in range(1, max_matches + 1):
        headers.extend([f'Match_{i}_Margin_Short', f'Match_{i}_Points'])
    ...
    with open(output_file, 'w', encoding='utf-8-sig', newline='') as f:
        ...
        for entry in leaderboard_list:
            ...
            for i in range(1, max_matches + 1):
                match = entry['Matches'].get(i, {'Margin_Short': '---', 'Points': 0.0})
                row[f'Match_{i}_Margin_Short'] = match['Margin_Short']
                row[f'Match_{i}_Points'] = match['Points']
            writer.writerow(row)

    print(f"Detailed margin poll leaderboard created: {output_file}")
    print(f"Processed {len(week_files)} weeks, {max_matches} matches, {len(combined_data)} users")
    return output_file

if __name__ == "__main__":
    output_file = generate_detailed_margin_leaderboard()
    if output_file:
        print(f"Detailed margin poll leaderboard generation completed for {WEEK}")